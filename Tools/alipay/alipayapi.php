...
/**********************数据记录*******************/
//记录到数据表中
$mysqli = new mysqli('localhost','root','123456','fs2');
//判断是否连接成功
...
//写入数据.先判断订单是否存在
$data = $mysqli -> query("select id from sp_alipay where order_id = '$out_trade_no'");
if($data -> num_rows){
	die('请勿重复创建订单');
}
$time = time();
//写入数据
$rst = $mysqli -> query("insert into sp_alipay (order_id,order_name,fee,body,status,created_at) values ('$out_trade_no','$subject','$total_fee','$body','0',$time)");
if(!$rst){
	die('订单串讲失败');
}
